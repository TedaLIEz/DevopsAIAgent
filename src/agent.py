...
class Tagging(BaseModel):
    ...
    status: str = Field(
        description="The status of the tagging log. Select from 'success', 'failure', 'pending', 'timeout', or 'unknown' if user input is not a log.")
    error_info: str = Field(
        description="The error information if the status is 'failure'. Please provide the error message in English.")
...
class Agent:
    ...
    def analysis(self, message):
        ...
        tagging_chain = prompt | self.__model | PydanticToolsParser(
